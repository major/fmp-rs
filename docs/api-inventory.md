...
| Price target | `price-target?symbol=AAPL` | unavailable | Stable docs expose `price-target-consensus` and `price-target-summary`; the generic historical price target endpoint is legacy-only, so `analyst price-target` now returns a local `endpoint_unavailable` error instead of calling a known-bad stable path. |
| Company outlook | `company-outlook?symbol=AAPL` | unavailable | FMP only documents this aggregate endpoint as legacy; `company outlook` now returns a local `endpoint_unavailable` error instead of calling a known-bad stable path. |
| Market open | `all-exchange-market-hours` | confirmed | Implemented as `calendar market-hours` using the current stable all-exchange market hours path. |
...
| Company/reference | [Company outlook](https://site.financialmodelingprep.com/developer/docs/company-outlook-api) | unavailable | high | Legacy-only aggregate endpoint. `company outlook` returns `endpoint_unavailable`; use individual stable commands such as `company profile`, `fundamentals income-statement`, `fundamentals ratios`, and `company scores`. |
...
| Quote/market | [Realtime stock quote](https://site.financialmodelingprep.com/developer/docs/stable/quote) | confirmed | high | Current stable docs map real-time quote data to `quote?symbol=AAPL`; `market realtime-quote` uses the same stable path as `market quote`. |
...
| Quote/market | [Stock price change](https://site.financialmodelingprep.com/developer/docs/stock-price-change-api) | confirmed | high | Tested `stock-price-change?symbol=AAPL`; implemented as `market price-change`. Multi-symbol calls use comma-separated `symbol=AAPL,MSFT` rather than `symbols=...`. |
...
| Ratios/metrics/growth | [Financial statement growth](https://site.financialmodelingprep.com/developer/docs/stable/financial-statement-growth) | confirmed | high | Current stable path is `financial-growth?symbol=AAPL&period=annual&limit=5`; implemented as `fundamentals statement-growth`. |
...
| Calendars/corporate actions | [Earnings surprises](https://site.financialmodelingprep.com/developer/docs/earnings-surprises-api) | unavailable | low | Symbol earnings surprises are legacy-only in current docs. `analyst earnings-surprises` returns `endpoint_unavailable` instead of calling a known-bad stable path. |
...
| Calendars/corporate actions | [Market open](https://site.financialmodelingprep.com/developer/docs/stable/all-exchange-market-hours) | confirmed | low | Implemented as `calendar market-hours` via `all-exchange-market-hours`. |
...
| Analyst/ratings/price targets | [Price target](https://site.financialmodelingprep.com/developer/docs/price-target-api) | unavailable | medium | Historical price targets are legacy-only in current docs. `analyst price-target` returns `endpoint_unavailable`; use `analyst price-target-consensus` or `analyst price-target-summary` for stable data. |
...
| Analyst/ratings/price targets | [Upgrades and downgrades](https://site.financialmodelingprep.com/developer/docs/upgrades-and-downgrades-api) | unavailable | medium | Upgrades and downgrades are legacy-only in current docs. `analyst upgrades-downgrades` returns `endpoint_unavailable`; use `analyst grades` for stable analyst grade actions. |
