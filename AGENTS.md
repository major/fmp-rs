...
- Keep self-discovery cheap and reliable: `fmp-agent schema`, `fmp-agent commands`, and `fmp-agent completions <shell>` must require no API key, make no network requests, and stay aligned with the real command surface.
