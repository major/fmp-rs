...
- `build.rs` generates `man/fmp-agent.1` from `Cli::command()` via `clap_mangen` when the `cli` feature is active; skipped for library-only builds. The `man/` directory is gitignored and included in cargo-dist releases via `dist-workspace.toml`.
