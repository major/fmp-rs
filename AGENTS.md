...
- For new endpoints, preserve the pattern: add an `Endpoint` constant in `src/endpoint.rs`, add a flat `Command` variant in `src/cli/args.rs`, and dispatch through the matching shape helper from `src/cli/dispatch.rs`. Avoid bespoke per-endpoint client methods.
