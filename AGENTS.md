...
- `src/cli/` owns command parsing and stdout JSON shape. `mod.rs` is the public entry point for `Cli` and `run`, `args.rs` owns internal Clap command/argument definitions, `help.rs` owns shared user-facing help strings used by Clap and man-page generation, `commands.rs` owns command dispatch, `dispatch.rs` owns shape helpers (`run_endpoint`, `run_query`, `run_by_symbol`, `run_by_symbol_limit`, `run_by_symbol_date_range`, `run_by_date_range`, `run_by_name_date_range`, `run_annual`, `run_annual_report_form`, `run_technical_sma`, `run_news`, `run_paged`), `output.rs` owns compact success rendering, and `tests.rs` keeps CLI parser/dispatch tests. User-facing commands are flat domain-prefixed verbs (`company-profile`, `market-quote`, `fundamentals-income-statement`, `analyst-grades`, `economic-indicators`, `technical-sma`, `news-stock`, etc.); old grouped commands and legacy aliases are rejected. Successful CLI output is the raw FMP JSON payload on one line and no output formatting or filtering options are available.
...
- `examples/generate-man.rs` generates `target/generated-man/man/fmp-agent.1` from `Cli::command()` via `clap_mangen` for release packaging. The generated `target/generated-man/man/` directory is included in cargo-dist releases via `dist-workspace.toml`; normal Cargo builds do not write man pages into the source tree.
...
- Release automation uses `cliff.toml` for changelog generation, `dist-workspace.toml` for cargo-dist artifact builds, `.github/workflows/release.yml` for CI (artifacts + crates.io OIDC publish), and the `/make-release` opencode command to bump, changelog, commit, and tag locally before pushing. Release CI runs `cargo run --locked --example generate-man --features cli -- target/generated-man/man/fmp-agent.1` before `dist build` so archives include the man page without a build-script side effect.
...
- For new endpoints, preserve the pattern: add an `Endpoint` constant in `src/endpoint.rs`, add centralized help strings in `src/cli/help.rs`, add a flat `Command` variant in `src/cli/args.rs`, and dispatch through the matching shape helper from `src/cli/dispatch.rs`. Avoid bespoke per-endpoint client methods.
...
- Run `cargo run --locked --example generate-man --features cli -- target/generated-man/man/fmp-agent.1` to generate the release man page locally. Do NOT commit generated man-page output.
- `help_heading` on `Subcommand` enum variants is NOT supported in Clap 4.6. The flat `Commands:` listing in `--help` is intentional. Do NOT add `#[command(help_heading = "...")]` to `Command` enum variants; it compiles but has no effect and creates misleading expectations.
...
- `fmp-agent schema` dumps CLI metadata as versioned JSON (`schema_version: 1`, experimental). It bypasses the api_key validation block in `src/cli/mod.rs::run()` via an early-return branch at lines 27-33: `if let args::Command::Schema = &cli.command { ... return Ok(()); }`. This branch runs before `MissingApiKey` is checked, so `schema` works without any API key configured.
- When adding a new `Command` variant, you MUST also add it to `tests/cli_drift.rs::COMMANDS` and supply both `about` and `long_about` (with an `Examples:` block) in `src/cli/help.rs`. The `cli_drift` test fails if any variant is missing from the constant list or if either help field is absent.
