...
- The `Error` enum has an `exit_code()` method that returns distinct `ExitCode` values per variant: 0 success, 2 usage error (`MissingArgument`), 3 config error (`MissingApiKey`/`InvalidBaseUrl`), 4 network error (`Http`), 5 API error (`Api`/`RateLimited`), 6 parse error (`Json`). HTTP 429 uses the stable `rate_limited` kind while keeping exit code 5. The exit codes are documented in `--help` via `after_help` on `Cli`.
