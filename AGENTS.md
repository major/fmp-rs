...
## LLM and agent ergonomics

- Treat LLM/tool-calling usability as a core CLI design requirement. `fmp-agent` should be as easy for a large LLM to discover, invoke, parse, and recover from as familiar shell tools like `git`, `gh`, and `rg`.
- Preserve the git-like grouped command tree, predictable JSON stdout, structured JSON runtime errors on stderr, semantic exit codes, and stdout/stderr separation. Agents should branch on exit codes and machine-readable output instead of scraping prose.
- Keep self-discovery cheap and reliable: `fmp-agent schema`, `fmp-agent commands`, `fmp-agent completions <shell>`, and `fmp-agent help <topic>` must require no API key, make no network requests, and stay aligned with the real command surface.
- Prefer reusable argument shapes and centralized help/schema metadata over bespoke endpoint behavior. Shape-based dispatch lets LLMs generalize from one command to the next without guessing flags.
- Do not add interactive prompts, pagers, hidden side effects, decorative output, or new formatting/filtering layers. Keep the CLI composable with shell pipelines such as `jq`, scripts, and tool runners.
...
- Income statement, balance sheet, and cash flow fundamentals default `--period annual --limit 5` and accept `--period quarter`; other annual fundamentals endpoints default `--limit 5`; stock and paginated news endpoints default `limit` to 10; paginated endpoints default `page` to 0. SEC filings default `--from` to 90 days ago (via `jiff`) when the user omits it, because the FMP API requires `from` for this endpoint.
- Annual endpoints include statements, as-reported statements, statement growth, enterprise values, analyst estimates, and annual report form JSON. Symbol-only reference/metric endpoints include profile, key executives, peers, ETF holdings, dividends, splits, financial scores, share float, report dates, price target consensus/summary, analyst grades, and rating consensus. `market price-change` accepts one or more symbols and sends them as a comma-joined `symbol` value because the FMP stable endpoint uses `symbol=AAPL,MSFT` rather than `symbols=...`. Historical ratings and fundamentals earnings use the symbol plus limit shape. Commands whose FMP docs are legacy-only (`company outlook`, `analyst price-target`, `analyst upgrades-downgrades`, and `analyst earnings-surprises`) return a local `endpoint_unavailable` error with exit code 5 rather than making a known-bad stable API request.
...
- The `Error` enum has an `exit_code()` method that returns distinct `ExitCode` values per variant: 0 success, 2 usage error (`MissingArgument`), 3 config error (`MissingApiKey`/`InvalidBaseUrl`), 4 network error (`Http`), 5 API or unavailable endpoint error (`Api`/`RateLimited`/`EndpointUnavailable`), 6 parse error (`Json`), 7 strict empty symbol result (`EmptyResult`). HTTP 429 uses the stable `rate_limited` kind while keeping exit code 5. Legacy-only commands use the stable `endpoint_unavailable` kind while keeping exit code 5. The exit codes are documented in `--help` via `after_help` on `Cli`.
- Empty JSON arrays remain successful raw API responses by default because FMP can validly return no rows. The global `--strict-empty` flag only turns empty payloads tagged by symbol lookup dispatch helpers into `empty_result` runtime errors and suggests `fmp-agent search <SYMBOL>`.
...
- `fmp-agent schema` dumps CLI metadata as versioned JSON (`schema_version: 3`, experimental). It bypasses the api_key validation block in `src/cli/mod.rs::run()` via an early-return branch that runs before `MissingApiKey` is checked, so `schema` works without any API key configured. `fmp-agent doctor` also bypasses API key validation and emits local-only JSON readiness fields (`ok`, `version`, sanitized `base_url`, `api_key`, and `live_connectivity.checked: false`) without making network requests or revealing secrets. `fmp-agent help <topic>` is also metadata-only and currently supports `environment`, `exit-codes`, `schema`, `troubleshooting`, and `examples`. The troubleshooting topic explains legacy endpoint `api_error` HTTP 404 output from older installed CLIs, the current `endpoint_unavailable` behavior, and stable fallback commands such as `analyst grades SYMBOL`. Bare-group invocations (a group name with no subcommand, including `help`) also bypass the api_key check and print the group's help text with exit 0, so users can explore available subcommands without configuring a key.
