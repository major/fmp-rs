...
- `examples/generate-man.rs` generates `man/fmp-agent.1` from `Cli::command()` via `clap_mangen`. The man page is committed to the repo and regenerated locally when CLI arguments change.
...
- Run `cargo run --locked --release --example generate-man --features cli -- man/fmp-agent.1` to regenerate the man page locally when CLI arguments change. Commit the updated `man/fmp-agent.1`.
